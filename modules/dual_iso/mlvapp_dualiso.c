...
/* SNAP-R: reduce horizontal row bleed at shadow edges (MLV App port) */
...
#pragma omp atomic
                        ...
                        deep_shadow++;
                        if (diso_ev_gradient_at(gray, raw2ev, w, x, y) > EV_RESOLUTION/3)
                        {
                            dmin = d0;
                            dmax = d0;
                        }
...
int diso_get_full20bit(struct raw_info raw_info, uint16_t * image_data, int dark_frame, int iso1, int iso2, int * iso_pattern, int * auto_correction, double * ev_correction, int * black_delta, int interp_method, int use_alias_map, int use_fullres, int chroma_smooth_method, int threads)
{
    ...
    {
        static int snap_raw2ev[1<<20];
        static int snap_ev2raw_0[24*EV_RESOLUTION];
        build_ev2raw_lut(snap_raw2ev, snap_ev2raw_0, black, white);
#ifndef STDOUT_SILENT
        ...
        diso_stripe_fix_robust(w, h, dark, bright, white_darkened,
            ...
            raw_info.active_area.y1, raw_info.active_area.y2, snap_raw2ev);
    }
    ...
    if (mix_images(raw_info, fullres, fullres_smooth, halfres, halfres_smooth, alias_map, dark, bright, overexposed, dark_noise, white_darkened, corr_ev, lowiso_dr, black, white, chroma_smooth_method))
    {
...

        {
            ...
            static int snap_ev2raw_0[24*EV_RESOLUTION];
            build_ev2raw_lut(snap_raw2ev, snap_ev2raw_0, black, white);
            int * snap_ev2raw = snap_ev2raw_0 + 10*EV_RESOLUTION;
            double * fullres_curve = build_fullres_curve(black);
#ifndef STDOUT_SILENT
            printf("Row bleed repair...\n");
#endif
            diso_repair_row_bleed(w, h, raw_buffer_32, snap_raw2ev, snap_ev2raw, black, dark_noise, fullres_curve);
        }
